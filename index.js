...
app.get('/', (req, res) => {
    ...
    <style>
        ...
        /* Carga el archivo fondo.jpg que subiste a tu repositorio */
        body::before {
            content: "";
            position: absolute;
            top: 0; left: 0; width: 100%; height: 100%;
            background-image: url('https://raw.githubusercontent.com/andreavalencia6012-arch/betflix-mexico-panel/main/fondo.jpg');
...
app.post('/admin/add-mail-masivo', (req, res) => {
    ...
    db.all("SELECT c.email, COALESCE(u.user, 'Fantasma/Eliminado') as owner FROM correos c LEFT JOIN usuarios u ON c.user_id = u.id", [], (err, rows) => {
        ...
        list.forEach(m => {
            ...
            if (correosExistentes[correoLimplio]) repetidos.push(`${correoLimpio}`);
